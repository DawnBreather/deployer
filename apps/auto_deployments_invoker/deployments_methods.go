...
func (d Deployments) submitAutodeploy(replacement ArtifactReferenceReplacement, git *gitwrapper.GitWrapper) (anyOk bool) {
  ...
  for deploymentName, deploymentObject := range d {
    ...
    if shouldAdjustDeployment, tiersForAutodeploy := filterTiersApplicableForAutodeploy(deploymentObject); shouldAdjustDeployment {
      if resultingReport[deploymentName] = adjustDeployment(deploymentName, deploymentObject, replacement, tiersForAutodeploy, git); resultingReport[deploymentName] {
        anyOk = true
      }
    }
  }
...
func adjustDeployment(deploymentName string, deploymentObject *deployment.Deployment, replacement ArtifactReferenceReplacement, tiersForAutodeploy map[string]*deployment.Tier, git *gitwrapper.GitWrapper) (ok bool) {

  secretsYamlBytes := getAdjustedSecrets(deploymentName, deploymentObject.Secrets, replacement)
...
func getAdjustedSecrets(deploymentName string, secrets deployment.Secrets, replacement ArtifactReferenceReplacement) (secretsYamlBytes []byte) {
  ...
  secretsYamlAdjustedBytes := replacement.ReplaceAll(secretsYamlBytes)
  if err != nil {
