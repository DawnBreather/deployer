...
type EnvironmentsConfigurations struct {
	...
	versionRefReplacement        VersionReferenceReplacement
...
func (ec *EnvironmentsConfigurations) Initialize() *EnvironmentsConfigurations {
	...
	ec.versionRefReplacement.InitializeForGitlabCiWorkflow()
...
func (ec *EnvironmentsConfigurations) SubmitAutodeploy() (exitCode int) {
	...
	if ok := ec.deployments.submitAutodeploy(ec.artifactReferenceReplacement, ec.versionRefReplacement, ec.gitWrapper); ok {
		exitCode = 0
	}
